  ...
  return (
    <div className="flex w-full flex-col gap-5 px-4 pt-1 sm:px-5 lg:px-7 2xl:px-9">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="m-0 text-xl font-bold text-void">
          {t("portfolio.title")}
        </h1>
        {/*
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard
          ...
          value={money(s?.buyingPowerCents ?? 0)}
        />
        <StatCard
          loading={isLoading}
      ...
      <div className="grid min-w-0 items-start gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
        ...
        {portfolio?.allocation?.length > 0 && (
          <section className="min-w-0 rounded-xl border border-cool-grey p-4 sm:p-5 overflow-hidden">
            <h2 className="mb-4 text-md font-bold text-void">
...
function HoldingsTable({ holdings, onSelect }) {
  ...
  return (
    <section className="min-w-0 rounded-xl border border-cool-grey overflow-hidden">
      <h2 className="px-5 py-4 text-md font-bold text-void">
          ...
          <tbody>
            {holdings.map((h) => (
              ...
              >
                ...
                <td
                  className={`${td} text-right font-numeric font-semibold tabular-nums`}
                >
    ...
    return (
      <div className="rounded-xl border border-cool-grey px-6 py-10 text-center">
        ...
        <p className="mx-auto mt-1 max-w-80 text-xs text-text-muted">
          Your first $10,000 is already in your account. Pick a stock to place
          your first order.
        </p>
        <Link
          to="/markets"
...
function HoldingCard({ holding: h, active, onSelect }) {
  ...
  const spark = Array.from({ length: 12 }, (_, i) => {
    ...
    return (
      h.avgCostCents +
      (nowPerUnit - h.avgCostCents) *
        t *
        (0.75 + 0.5 * Math.abs(Math.sin(i * 1.7)))
    );
  });
  ...
  return (
    <button
      ...
      className={[
        ...
        active
          ? "border-void/25 bg-hover/50"
          : "border-cool-grey hover:bg-hover/40",
      ].join(" ")}
    >
    ...
    <section className="rounded-xl border border-cool-grey p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          />
          <div>
            <div className="text-md font-bold text-void">
            ...
            {data && (
              <span
                className={`rounded-md px-1.5 py-0.5 font-numeric text-2xs font-semibold tabular-nums ${
                  data.changePct >= 0
                    ? "bg-green-tint text-gain"
                    : "bg-red-tint text-loss"
                }`}
              >
                {pct(data.changePct)}
              </span>
          ...
          {RANGES.filter((r) => !(isForex && r === "1D")).map((r) => (
            <button
              ...
              className={[
                ...
                r === range
                  ? "bg-ink text-white"
                  : "text-text-muted hover:text-void",
              ].join(" ")}
            >
              {RANGE_LABEL[r]}
            </button>
      ...
      */}
      ...
      <p className="mt-3 mb-0 text-2xs text-text-muted">
        ...
        {!tradable && data
          ? ` ${symbol} is ${String(data.status).toLowerCase()} - not tradable.`
          : ""}
      </p>
    </section>
  );
  ...
  return (
    <section className="rounded-xl border border-cool-grey p-5">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-md font-bold text-void">
            ...
            >
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="font-mono text-xs font-bold text-void">
                    {s.symbol}
                  </div>
              ...
              <button
                type="button"
                onClick={() =>
                  onRemove({ assetClass: s.assetClass, symbol: s.symbol })
                }
