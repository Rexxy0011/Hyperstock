...
export default function TraderOverrideModal({ open, onClose, trader }) {
  ...
  const filteredAssets = useMemo(() => {
    ...
    return allAssets.filter(
      (a) => (a.assetClass || "stocks").toLowerCase() === categoryFilter
    );
  ...
  const liveReturnPct =
    trader?.computed?.returnPct != null
      ? trader.computed.returnPct
      : heldList.length > 0
        ? heldList.reduce((sum, h) => sum + (h.returnPct || 0), 0)
  ...
  const orderPriceCents =
    selectedAsset?.priceCents || selectedAsset?.priceUsdCents || 0;
  ...
  const addFundsMutation = useMutation({
    ...
    onSuccess: () => {
      notify.success(
        `Added ${money(Math.round(Number(fundAmount) * 100))} to buying power`
      );
      setFundAmount("");
      refetchPositions();
      queryClient.invalidateQueries({ queryKey: ["admin", "users"] });
  ...
  const placeTrade = useMutation({
    mutationFn: (data) =>
      post(`/admin/users/${trader.id}/portfolio/orders`, data),
    onSuccess: (data) => {
      notify.success(
        `Order filled: ${data.side} ${data.quantity} ${data.symbol}`
      );
      setShares("1");
      refetchPositions();
      queryClient.invalidateQueries({ queryKey: ["admin", "users"] });
      ...
      footer={
        <div className="flex w-full items-center justify-between">
          <span className="text-2xs text-text-muted">
            All actions immediately update live orders, ledger records, and
            rankings.
          </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    min={
                      selectedAsset?.assetClass === "stocks" ? "1" : "0.0001"
                    }
                    placeholder={
                      selectedAsset?.assetClass === "stocks"
                        ? "e.g. 10"
                        : "e.g. 0.25"
                    }
                    value={shares}
                    ...
                    {userHeldPosition ? (
                      <span>
                        ...
                        <strong className="text-void">
                          {userHeldPosition.shares} units (
                          {money(userHeldPosition.valueCents)})
                        </strong>
                      </span>
                    ) : (
            ...
            <div className="rounded-xl border border-cool-grey bg-white p-4 shadow-card">
              ...
              {heldList.length === 0 ? (
                <div className="py-6 text-center text-xs text-text-muted">
                  No active holdings. Choose an asset above to place a BUY
                  order.
                </div>
              ) : (
            ...
            <div className="rounded-xl border border-cool-grey bg-white p-5 shadow-card">
              <div className="flex items-center justify-between border-b border-cool-grey/60 pb-3 mb-4">
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-text-muted">
                    Credit buying power directly to this account with no upper
                    limit.
                  </p>
                </div>
              ...
              {Boolean(parseFloat(fundAmount || 0)) && (
                <div className="mt-4 rounded-lg bg-green-tint/50 border border-gain/20 p-3 text-xs text-gain">
                  ...
                  <strong>
                    {money(
                      currentCash + Math.round(parseFloat(fundAmount) * 100)
                    )}
                  </strong>{" "}
