...
export async function fetchRows() {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`coingecko provider HTTP ${res.status} — using fallback crypto fixtures`);
      return fallbackRows();
    }
    ...
    console.warn(`coingecko fetch error: ${err.message} — using fallback crypto fixtures`);
    return fallbackRows();
  }
}
