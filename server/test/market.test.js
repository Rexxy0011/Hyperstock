...
function seedFeed(rows) {
  liveFeed.subscriptions = new Map(
    rows.map((r) => [
      r.stream ?? r.symbol,
      { symbol: r.symbol, assetClass: r.assetClass ?? "stocks" },
    ])
  );
  liveFeed.prices = new Map(
    rows
      ...
      .map((r) => [
        r.stream ?? r.symbol,
        { price: r.priceCents / 100, priceCents: r.priceCents, at: r.at },
      ])
  );
}
...
test("tick flush", async (t) => {
  ...
  await t.test("writes a tick that is inside the refresh window", () => {
    seedFeed([
      { symbol: "AAPL", priceCents: 31_000, at: NOW.getTime() - 3_000 },
    ]);
  ...
  await t.test("the derived percentage is arithmetically right", () => {
    ...
    const derived =
      ((priceCents - previousCloseCents) / previousCloseCents) * 100;
    ...
    seedFeed([
      {
        ...
        symbol: "BTC",
        assetClass: "crypto",
        priceCents: 7_890_600,
        at: NOW.getTime(),
      },
      {
        stream: "OANDA:EUR_USD",
        symbol: "EURUSD",
        assetClass: "forex",
        priceCents: 117,
        at: NOW.getTime(),
