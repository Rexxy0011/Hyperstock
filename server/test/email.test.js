...
test("emails", async (t) => {
  await t.test("each OTP type gets its own subject and reason", () => {
    ...
    assert.match(verify.subject, /verification code/i);

    assert.match(reset.html, /Reset your password/i);
    assert.match(signIn.html, /Sign in to your account/i);
    assert.match(verify.html, /Verify your email address/i);
  });
  ...
  await t.test(
    "the unsubscribe link is keyed on the token, never the address",
    () => {
      const { text, html } = newsletterEmail({
        subject: "x",
        body: "y",
  ...
  await t.test("messages carry hyperstocks.finance link tag in footer", () => {
    ...
    for (const { text, html } of [otp, news]) {
      ...
      assert.match(
        html,
        /<a href="https:\/\/hyperstocks\.finance"[^>]*>hyperstocks\.finance<\/a>/i
      );
    }
  });
